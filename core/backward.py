...
def __linear_backward(dZ, D, linear_cache, keep_dims):
    A_prev, W, b = linear_cache
    m = A_prev.shape[1]
    dW = np.dot(dZ, A_prev.T) / m  # derivada de W
    db = np.sum(dZ, axis=1, keepdims=True) / m  # derivada de b
    dA_prev = np.dot(W.T, dZ) * D
    dA_prev = dA_prev / keep_dims

    return dA_prev, dW, db


def linear_backward_activation(dA, cache_tuple, activation: Enum, keep_dims):
    linear_cache, activation_cache, D = cache_tuple
    if activation == Activation.SIGMOID:
        dZ = sigmoid_backward(dA, activation_cache)
    elif activation == Activation.RELU:
        dZ = relu_backward(dA, activation_cache)
    dA_prev, dW, db = __linear_backward(dZ, D, linear_cache, keep_dims)

    return dA_prev, dW, db


def L_model_backward(AL, Y, caches, keep_dims):
    grads = {}
    L = len(caches)
